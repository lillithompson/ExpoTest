...
- UGC file thumbnails (Expo Go iOS): Line width must match built-in tile sets. `strokeScaleByName` (from user tile set resolution) scales stroke for palette and canvas. File list thumbnails now use the same scale: (1) When showing the saved thumbnail image on iOS, the app prefers rendering the tile grid with scaled `strokeWidth` (file.lineWidth * strokeScaleByName) so UGC tiles match; (2) When showing the fallback grid (no thumbnail) on native, each tile uses scaled strokeWidth; (3) Download overlay tiles use scaled strokeWidth. This fixes file thumbnails showing un-adjusted line widths on iOS.
